...
train_path = 'D:/teamproject/breed'
...
def face_detector(path, w, h):
    ...
    for idex, folder in enumerate(folder_name):                   # 폴더별 이미지 불러오기
        ...
        np.save('D:/teamproject/data/face_label_%s.npy'%(folder), target)
        print('---------- END %s ---------'%(folder))
...
def _append(image_label, shape):                     # shape : 저장한 파일 shape / 행은 무조건 0
    ...
    for f in folder_name:
        y = np.load('D:/teamproject/data/face_%s_%s.npy'%(image_label, f))
        print(y.shape)
        arr = np.append(arr, y, axis = 0)

    print(arr.shape)

    np.save('D:/teamproject/data/face_%s_part1.npy'%(image_label), arr)
    print('------------- save complete ---------------')
