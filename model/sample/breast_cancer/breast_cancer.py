...
# model.save('./model/sample/breast_cancer/breast_cancer_model_test01.h5')
...
model.compile(loss = 'binary_crossentropy',
              metrics = ['accuracy'],
              optimizer = 'adam')
modelpath = './model/sample/breast_cancer/breast_cancer_check-{epoch:02d} - {val_loss:.4f}.hdf5'
checkpoint = ModelCheckpoint(filepath=modelpath, monitor='val_loss', verbose = 1,
                                  save_best_only=True, save_weights_only=False)

hist = model.fit(x_train, y_train, callbacks = [es,checkpoint],
                 epochs = 200, batch_size = 1,
                 validation_split = 0.2, verbose = 1)

# model.save_weights('./model/sample/breast_cancer/breast_canversave_weight1.h5')
