...
# model.save('./model/sample/diabets/diabets_model_test01.h5')
...
model.compile(loss = 'mse',
              metrics = ['mse'],
              optimizer = 'adam')
modelpath = './model/sample/diabets/diabets_check-{epoch:02d} - {val_loss:.4f}.hdf5'
checkpoint = ModelCheckpoint(filepath=modelpath, monitor='val_loss', verbose = 1,
                                  save_best_only=True, save_weights_only=False)

model.fit(x_train, y_train, callbacks = [es, checkpoint],
          epochs = 1000, batch_size = 2,
          validation_split = 0.05, verbose = 1)

# model.save_weights('./model/sample/diabets/diabets_weight1.h5')
